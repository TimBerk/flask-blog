...
if __name__ == "__main__":
    ...
    p1 = Post(title="Перестаньте использовать !important. Помогаем разобраться с каскадом CSS",
              text="<p>Когда CSS-правило не работает, многие разработчики хотят добавить !important и забыть о проблеме. Рассказываем, почему так делать не стоит и как правильно работать с CSS.</p>")
    p1.author = admin
    ...
    p2 = Post(title="Facebook выпустит приложение для стриминга игр",
              text="<p>Компания Facebook выпустит приложение для стриминга игр, которое должно стать конкурентом для стриминговых платформ Youtube и Twitch, сообщает NYT, ссылаясь на руководителя приложения Фиджи Симо.</p>")
    p2.author = admin
    ...
    p3 = Post(title="Google вводит новые правила в Play Store",
              text="<p>Google Play станет более удобной и &amp;quot;прозрачной&amp;quot; для пользователей. Компания Google планирует обязать разработчиков предоставлять необходимую информацию о бесплатном периоде действия программ, сроках оплаты приложений и дате окончания подписки в простом и понятном для пользователей виде.</p>")
    p3.author = admin
